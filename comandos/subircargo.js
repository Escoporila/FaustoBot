module.exports.run = async ({Discord, client, message, args, low, FileSync, adapter, db}) => {
    ...
    let cargoAtual = cargo(cargos, message.member) || 1;
    ...
    if(npc < cargos.length)
    {
        ...
        {
            message.member.removeRole(cargoAtual);
            message.member.addRole(cargos[npc].id);
...
function cargo(cargos, membro)
{
    ...
    cargos.forEach(cargo => {
        if(membro.roles.has(cargo.id)) cargoAtual = cargo.id;
    });
    return cargoAtual;
}